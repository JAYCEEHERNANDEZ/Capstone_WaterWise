...
export default function EventRecordsTable({ events = sampleEvents, onEdit, onDelete }) {
  return (
    <Table
      ariaLabel="Event records"
      columns={[
        { key: "event", label: "Event" }, { key: "schedule", label: "Schedule" },
        { key: "location", label: "Location" }, { key: "status", label: "Status" },
        { key: "tags", label: "Tags" }, { key: "actions", label: "Actions", className: "text-right" },
      ]}
      data={events}
      emptyDescription="Create an event to add it to the community calendar."
      emptyTitle="No events scheduled"
      getRowKey={(event) => event.id}
      header={<div className="flex items-center gap-3 border-b border-slate-200 px-5 py-4">
