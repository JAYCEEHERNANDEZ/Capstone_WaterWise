...
export default function AnnouncementPage({
  ...
  return (
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-[0_20px_60px_rgba(15,23,42,0.06)] sm:p-6">
      ...
      {announcements.length === 0 ? (
        ...
        <div className="space-y-4">
          {announcements.map((announcement) => (
            <article
              className="group rounded-2xl border border-slate-200 p-5 transition hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-lg"
              key={announcement.id}
            >
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      <span className="inline-flex rounded-full bg-sky-50 px-2.5 py-1 text-[11px] font-bold uppercase tracking-wide text-sky-700">
                  ...
                  {(onEdit || onDelete) && (
                    <div className="mt-4 flex gap-2">
                      {onEdit && (
                        <button className="rounded-lg bg-sky-50 px-3 py-2 text-sm font-bold text-sky-700" onClick={() => onEdit(announcement)} type="button">
                          Edit
                        </button>
                      )}
                      {onDelete && (
                        <button className="rounded-lg bg-red-50 px-3 py-2 text-sm font-bold text-red-700" onClick={() => onDelete(announcement.id)} type="button">
                          Delete
                        </button>
                      )}
                    </div>
