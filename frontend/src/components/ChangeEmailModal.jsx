...
export default function ChangeEmailModal({ accountRole = "consumer", isOpen, onClose, onSuccess }) {
  ...
  const sendOtp = async () => {
    ...
    try {
      ...
      setChallengeToken(result.challengeToken); setMaskedEmail(result.maskedEmail); setMessage(result.message); setOtp("");
  ...
  return (
    <Modal description="Verify your current registered email before replacing it." eyebrow="Account security" isOpen={isOpen} onClose={onClose} size="sm" title="Change email address">
      <div className="p-5 sm:p-6">
        {!challengeToken && <div className="text-center"><span className="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-water-50 text-water-700"><Mail className="h-5 w-5" /></span><p className="mt-3 text-sm leading-6 text-slate-600">We’ll send a 6-digit verification code to your current registered email address.</p><button className="ww-primary-button mt-5 flex min-h-12 w-full items-center justify-center gap-2" disabled={isSubmitting} onClick={sendOtp} type="button">{isSubmitting && <LoaderCircle className="h-5 w-5 animate-spin" />}{isSubmitting ? "Sending code…" : "Send verification code"}</button></div>}

        {challengeToken && !emailChangeToken && <form className="space-y-4" onSubmit={verifyOtp}><p className="text-sm text-slate-600">Enter the code sent to <strong>{maskedEmail}</strong>.</p><div><label className="text-sm font-semibold" htmlFor="email-change-otp">Verification code</label><input autoComplete="one-time-code" className="ww-field mt-2 py-3 text-center font-mono text-xl tracking-[0.35em]" id="email-change-otp" inputMode="numeric" maxLength={6} onChange={(event) => { setOtp(event.target.value.replace(/\D/g, "")); setError(""); }} value={otp} /></div>{message && !error && <p className="rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-sm font-semibold text-emerald-800">{message}</p>}<button className="ww-primary-button flex min-h-12 w-full items-center justify-center gap-2" disabled={isSubmitting} type="submit">{isSubmitting && <LoaderCircle className="h-5 w-5 animate-spin" />}{isSubmitting ? "Verifying…" : "Verify current email"}</button><button className="w-full text-sm font-bold text-water-700" disabled={isSubmitting} onClick={sendOtp} type="button">Resend code</button></form>}
