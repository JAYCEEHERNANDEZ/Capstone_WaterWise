  ...
  return (
    <section className="ww-glass-strong overflow-hidden rounded-[24px]">
      <div className="border-b border-slate-200/70 p-5 sm:p-6">
        <p className="ww-eyebrow !text-sky-700">Step 1 of 4</p>
      ...
      <div className="max-h-[46rem] space-y-3 overflow-y-auto p-4 sm:p-6">
        {consumers.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-slate-300 bg-white/55 p-8 text-center">
          ...
          <button
            className={`flex min-h-24 w-full items-center gap-4 rounded-2xl border p-4 text-left transition sm:p-5 ${selectedId === consumer.id ? "border-sky-500 bg-sky-50 ring-4 ring-sky-100" : "border-slate-200 bg-white/75 hover:border-sky-300 hover:bg-sky-50/70"}`}
            key={consumer.id}
